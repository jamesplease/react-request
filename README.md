...
| persistStore                         |                  | Optional object used to persist the cache. Works with react-native AsyncStorage and window.localStorage, but any implementation exposing setItem and getItem can be used. The cache is hydrated on mount, and persisted on each new request. |
